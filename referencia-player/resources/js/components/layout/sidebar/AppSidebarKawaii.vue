    ...
    >
        <div class="flex h-[72px] shrink-0 items-center justify-between px-5">
            ...
            <button
                v-if="isMobile"
                type="button"
                class="kawaii-icon-btn ml-1 flex h-8 w-8 shrink-0 items-center justify-center"
                ...
                <template v-for="(item, index) in navItems" :key="item.separator ? `sep-${index}` : (item.href ?? index)">
                    ...
                    <li v-else>
                        <Link
                            ...
                            :prefetch="panelNavPrefetch"
                            :class="[
                                'kawaii-nav-link flex items-center gap-3 px-3 py-2.5 text-[13px] font-semibold transition-colors',
                                isActive(item.href) ? 'kawaii-nav-active' : '',
                            ]"
                        >
                            <component
                                :is="item.icon"
                                class="kawaii-nav-icon h-[17px] w-[17px] shrink-0"
                                aria-hidden="true"
                            />
